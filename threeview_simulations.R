...
#saveRDS(tv_sims, "simulations/results/threeview.rds")
...
v1v2 <- ggplot(data.df, aes(x=X1,y=X2, shape = c1, color =c2)) + geom_point() +
  theme_bw() +
...
v1v3 <- ggplot(data.df, aes(x=X1,y=X3,shape = c1, color =c3)) + geom_point() +
  theme_bw() +
...
v2v3 <- ggplot(data.df, aes(x=X2,y=X3,shape = c2, color =c3)) + geom_point() +
  theme_bw() +
