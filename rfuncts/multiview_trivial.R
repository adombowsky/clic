multiview_trivial <- function(n=200, omega0=1/2, seed=1, R=1000, B=500, Th = 1) {
  set.seed(seed)
  require(mcclust)
  require(mcclust.ext)
  require(Rcpp)
  ...
  for (i in 1:n) {
    u <- runif(1)
    X1[i] <- rnorm(n=1, mean = theta1[c1.true[i]], sqrt(0.2)) #+ ifelse(u<1/5, runif(1, min(theta1[c1.true[i]],0),   max(theta1[c1.true[i]],0)), 0)
    X2[i] <- rnorm(n=1, mean = theta2[c2.true[i]], sqrt(0.5)) #+ ifelse(u<1/5, runif(1, min(theta2[c2.true[i]],0),   max(theta2[c2.true[i]],0)), 0)
  }
  X <- cbind(X1,X2)
