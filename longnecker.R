...
CPPdata <- read.csv(file = "data/finaldde2.txt", header=T)
####Define data
set.seed(1996)
...
# X.samp[,1] = log(X.samp[,1])
# X.samp[,2] = log(X.samp[,2])
X.samp <- scale(X.samp)
#X.samp[,1] = (X.samp[,1] - mean(X.samp[,1]))/sd(X.samp[,1])
#X.samp[,2] = (X.samp[,2] - mean(X.samp[,2]))/sd(X.samp[,2])
...
data.plot <- ggplot(plot.df, aes(x=weight, y=gestation, shape = c1, col = c2)) + 
  ...
  labs(title = "Clusterings of the CPP Children",
       shape = TeX("$\\hat{C}_{1}$"),
       color = TeX("$\\hat{C}_{2}$")) +
  theme_bw() +
...
plot_grid(tp.rand, rand.corr.plot, tp.rho, rho.corr.plot,
          labels = c("(a)", "(b)", "(c)", "(d)"),
          nrow = 2)
