...
# multiview
c.multiview <- readRDS("simulations/results/multiview.rds")
## dependent
...
# conditional
c.conditional <- readRDS("simulations/results/conditional.rds")
## dependent
...
mv.dep.plot <- ggplot(multiview.dep, aes(x=X1,y=X2,shape=c1, color =c2)) + geom_point() +
  theme_bw() +
...
mv.ind.plot <- ggplot(multiview.ind, aes(x=X1,y=X2,shape=c1, color =c2)) + geom_point() +
  theme_bw() +
...
mv.triv.plot <- ggplot(multiview.triv, aes(x=X1,y=X2,shape=c1, color =c2)) + geom_point() +
  theme_bw() +
...
ggplot(rand.trace.df, aes(x=iter, y=rand)) + geom_point(color="red") +
  xlab("Iteration") +
  ylab("Rand Index") +
